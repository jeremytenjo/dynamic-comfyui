...
RUN set -eux; \
    ...
    if [ -z "${COMFYUI_INSTALL_VERSION}" ]; then \
        COMFYUI_INSTALL_VERSION="v0.26.0"; \
        ...
        echo "⚠️ COMFYUI_VERSION='${COMFYUI_INSTALL_VERSION}' is legacy; using pinned default v0.26.0."; \
        COMFYUI_INSTALL_VERSION="v0.26.0"; \
    else \
        echo "❌ COMFYUI_VERSION must be semver (example: 0.26.0 or v0.26.0). Got: '${COMFYUI_INSTALL_VERSION}'."; \
        exit 1; \
    fi; \
